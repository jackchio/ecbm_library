...
*例程说明  ：1.可以在ecbm_core.h里设置你使用的单片机型号，然后再次编译并下载。
			...
			4.本例中4个串口都是使用定时器2，所以波特率也必须相同。请前往uart.h参考图形化界面的设置。
...
void main(){			//main函数，必须的。
	...
	while(1){
		...
		uart_printf(1,"Hello,UART1 is runing at %d times.\r\n",(u16)count);//向串口1发送字符串和count的值。
		uart_printf(2,"Hello,UART2 is runing at %d times.\r\n",(u16)count);//向串口2发送字符串和count的值。
		uart_printf(3,"Hello,UART3 is runing at %d times.\r\n",(u16)count);//向串口3发送字符串和count的值。
		uart_printf(4,"Hello,UART4 is runing at %d times.\r\n",(u16)count);//向串口4发送字符串和count的值。
