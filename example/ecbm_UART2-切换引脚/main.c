...
*例程说明  ：1.可以在ecbm_core.h里设置你使用的单片机型号，然后再次编译并下载。
			...
			4.也可以在图形化界面设置输出引脚，之不过用代码的话不用跑到uart.h去看那么麻烦。
...
//	uart_set_io(2,UART2_PIN_P10_P11);//将串口2切换到P10和P11口
	uart_set_io(2,UART2_PIN_P40_P42);//将串口2切换到P40和P42口
	while(1){
		count++;//统计循环次数，也是为了演示uart_printf的功能。
		
		uart_printf(2,"Hello,ECBM is runing at %d times.\r\n",(u16)count);//向串口2发送字符串和count的值。
