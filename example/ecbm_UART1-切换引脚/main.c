...
*例程说明  ：1.可以在ecbm_core.h里设置你使用的单片机型号，然后再次编译并下载。
			...
			4.虽然可以在图形化界面设置输出引脚，但是串口1在开启自动下载功能后，不允许将串口移至其他引脚。
			不过可以用代码强行切换，切换后自动下载功能就会失效。
...
void main(){			//main函数，必须的。
	...
	uart_set_io(1,UART1_PIN_P30_P31);//将串口切换到P30和P31口
//	uart_set_io(1,UART1_PIN_P36_P37);//将串口切换到P36和P37口
//	uart_set_io(1,UART1_PIN_P16_P17);//将串口切换到P16和P17口
//	uart_set_io(1,UART1_PIN_P43_P44);//将串口切换到P43和P44口	
	while(1){
